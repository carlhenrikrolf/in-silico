...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 6, 1);
 	...
 	ivoc_help("help ?1 hd /home/hbpschool2016/Documents/NEURON_tutorial/modeldb-etoh/x86_64/IhDA.mod\n");
...
static int  rate (  double _lv ) {
   double _la ;
 linf = 1.0 / ( 1.0 + exp ( ( _lv - vh ) / k ) ) ;
   _la = alpt ( _threadargscomma_ _lv ) ;
   taul = bett ( _threadargscomma_ _lv ) / ( a0t * ( 1.0 + _la ) ) ;
    return 0; }
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  states();
 if(error){fprintf(stderr,"at line 47 in file IhDA.mod:\n	SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
