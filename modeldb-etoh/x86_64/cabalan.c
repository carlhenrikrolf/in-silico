...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 7, 5);
 	...
 	ivoc_help("help ?1 cabalan /home/hbpschool2016/Documents/NEURON_tutorial/modeldb-etoh/x86_64/cabalan.mod\n");
...
static int _reset;
static char *modelname = "Calcium ion accumulation without diffusion and buffering";
...
_MATELM1(0, 0) *= ( PI * diam * diam / 4.0);  }
 /* COMPARTMENT PI * diam * diam / 4.0 {
     cai }
   */
 /* ~ cai < < ( - fCa * ica * PI * diam * ( 1e4 ) / ( 2.0 * FARADAY ) )*/
 f_flux = b_flux = 0.;
 _RHS1( 0) += (b_flux =   ( - fCa * ica * PI * diam * ( 1e4 ) / ( 2.0 * FARADAY ) ) );
 /*FLUX*/
    } return _reset;
 }
 ...
 */
 /* ~ cai < < ( - fCa * ica * PI * diam * ( 1e4 ) / ( 2.0 * FARADAY ) )*/
 f_flux = b_flux = 0.;
 Dcai += (b_flux =   ( - fCa * ica * PI * diam * ( 1e4 ) / ( 2.0 * FARADAY ) ) );
 /*FLUX*/
